...
fn init_db() -> Connection {
    ...
    let conn = Connection::open("data/messages.db")
...
async fn get_messages(
    ...
    let mut stmt = db
        .prepare(
            "SELECT id, group_id, sender_id, sender_name, text, created_at
             FROM messages
             WHERE group_id = ?
...
pub async fn run_server(port: u16) {
    ...
    let (tx, _) = tokio::sync::broadcast::channel::<Message>(100);
    let db = init_db();
